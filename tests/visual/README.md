...
| `VISUAL.trade(pid?, id?, { to?, counter_of? })` | an incoming trade offer; reports the row colour, its kind (request / proposal / counter), the actions it shows, how many offers are on screen and the list's share of the viewport height (the phone cap is 35%). `to` (the viewer's id, or another seat's) renders a proposal aimed at that seat, `counter_of` a counter |
...
| `VISUAL.bank(counts?)` | sets the bank's counts through `updateBankSoc()` (default: ore out, sheep low) and reports the scoreboard's bank row: box, on screen, counts, which are marked out, one line, smallest font |
| `VISUAL.tradeDrawer(mode?)` | opens the trade drawer in `'players'` (default) or `'bank'` mode - or as a non-active player in `'propose'` mode or from a request's Counter action (`'counter'`) - and reports it: role and the modes offered, the stock ribbons in Bank mode, rect and viewport share, whether it stays on screen, how many scoreboard rows it covers, page/drawer overflow, smallest hit box across the cards and the deal's chips, the hand as the give row (per stack: cards, bank rate, glow, disabled, `tabIndex`; smallest stack hit box), what is staked, the guide line and the submit button's label and state |
